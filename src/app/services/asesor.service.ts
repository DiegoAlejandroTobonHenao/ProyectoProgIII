...
export class AsesorService {
  ...
  url: String = "http://localhost:3000/api/asesors";
  ...
  searchAsesor(identification: String) {
    this.loadAllAsesors().subscribe(data => { this.asesorList = data });
  ...
  updateAsesor(asesor: UserModel): Observable<UserModel> {
    ...
    if (!exists) {
      ...
      return this.http.put<UserModel>(`${this.url}/${tmp._id}`, {
  ...
  deleteAsesor(identification: String): Observable<UserModel> {
    this.loadAllAsesors().subscribe(data => { this.asesorList = data });
    let asesor = this.asesorList.find(a => a._id == identification);
    return this.http.delete<UserModel>(`${this.url}/${asesor._id}`)
  }
}
