...
export class CountryService {
  ...
  updateCountry(country: CountryModel):Observable<CountryModel>{
    ...
    if (!exists){
      ...
      return this.http.put<CountryModel>(`${this.url}/${tmp._id}`, {
        code: country.code,
        name: country.name
      }, 
      {
  ...
  deleteCountry(code: String):Observable<CountryModel>{
    this.loadAllCountries().subscribe(data => {this.countryList = data});
    let country = this.countryList.find(c => c._id == code);
    return this.http.delete<CountryModel>(`${this.url}/${country._id}`)
  }
}
